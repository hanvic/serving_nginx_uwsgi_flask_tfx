...
def serve_by_image(threshold, target_height, target_width, root_maxClsSize,tar_maxClsSize,root_target_features,tar_target_features,img):
    ...
    start = time.time()
    root_json_response = requests.post('http://192.168.80.5:8501/v1/models/root:predict', data=data, headers=headers)
    tar_json_response = requests.post('http://192.168.80.5:8501/v1/models/tar:predict', data=data, headers=headers)
